...
**Meeting features (2026-07 parity pass):** knock-to-join waiting room — first socket in a room is host (`rooms[path]` in `socketManager.js`: host/names/media/hands/pending); events: `join-call(path, username)`, `waiting-room`, `join-request(-cancelled)`, `admit-user`/`deny-user`, `admitted`, `join-denied`, `host-changed`, `media-state`, `raise-hand`, `reaction`. `user-joined` now carries a third `meta` arg. Frontend `VideoMeet.jsx` uses a `phase` state machine (lobby→waiting→call / denied) — the old `askForUsername` bool is gone; components: `ParticipantsPanel.jsx`, `InviteModal.jsx`. Headless smoke test of the protocol lives in the session scratchpad pattern (two socket.io-client nodes; run from `frontend/` so imports resolve).
